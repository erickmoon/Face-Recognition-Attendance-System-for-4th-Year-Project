...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM class_unit where id= ".$_GET['id']);
...
<div class="container-fluid">
    <form action="" id="manage-class_unit">
        ...
        <div class="form-group">
            ...
            <select name="unit_id" id="" class="custom-select select2">
                <option value=""></option>
                <?php
                $class = $conn->query("SELECT * FROM units order by unit asc");
                while($row=$class->fetch_assoc()):
                ?>
                <option value="<?php echo $row['id'] ?>" <?php echo isset($unit_id) && $unit_id == $row['id'] ? 'selected' : '' ?>><?php echo ucwords($row['unit']) ?></option>
                <?php endwhile; ?>
            </select>
        </div>
    </form>
...
<script>
    $('#manage-class_unit').on('reset',function(){
        $('#msg').html('')
        $('input:hidden').val('')
    })
    $('#manage-class_unit').submit(function(e){
        e.preventDefault()
        start_load()
        $('#msg').html('')
        $.ajax({
            url:'ajax.php?action=save_class_unit',
