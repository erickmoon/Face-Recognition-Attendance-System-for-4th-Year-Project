...
Class Action {
	...
	function save_unit(){
		...
		$data = " unit = '$unit' ";
		$data .= ", description = '$description' ";
		$check = $this->db->query("SELECT * FROM units where unit = '$unit' ".(!empty($id) ? ' and id!=$id ' : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function delete_unit(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM units where id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_student(){
		...
		//file_put_contents("test.txt", $file_name);
		...
		if(empty($id)){
			...
			//Update profile pic
			$temporal_student_id = str_replace("/", "-", $_POST['id_no']);
			...
			// if everything is ok, try to upload file
				...
				$fileExt = pathinfo($_FILES["student_photo"]['name'], PATHINFO_EXTENSION);
				try{
					unlink('../../images/'.$temporal_student_id.".".$fileExt);
				}catch(Exception $E){

				}
			  if (move_uploaded_file($_FILES['student_photo']['tmp_name'],'../../images/'.$temporal_student_id.".".$fileExt)) {


						date_default_timezone_set("Africa/Nairobi");
						$db_hostname = "localhost:3305";
						$db_password = "";
						...
						$temporal_student_id = $temporal_student_id.".".$fileExt;

			        	$stmt = $conn->prepare("UPDATE `students` SET `student_photo` = ? WHERE `id_no` = ?");
				        $stmt->bind_param("ss",$temporal_student_id,$student_id);
				        $stmt->execute(); 
			  } else {
			    	file_put_contents("final2.txt", "something goes wrong");
			  }
			}
			    ...
			    if ($uploadOk == 0) {
			        ...
			        if (
			            move_uploaded_file(
			                ...
			                "../../images/" . $temporal_student_id . "." . $fileExt
			            )
			        ) {
			            date_default_timezone_set("Africa/Nairobi");
			            $db_hostname = "localhost:3305";
			            ...
			            $stmt = $conn->prepare(
			                "UPDATE `students` SET `student_photo` = ? WHERE `id_no` = ?"
			            );
			            $stmt->bind_param("ss", $temporal_student_id, $student_id);
			            $stmt->execute();
			        } else {
	...
	function save_class_unit(){
		...
		$check = $this->db->query("SELECT * FROM class_unit where $data2 ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO class_unit set $data");
		}else{
			$save = $this->db->query("UPDATE class_unit set $data where id = $id");
		}
	...
	function delete_class_unit(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM class_unit where id = ".$id);
		if($delete){
			return 1;
		}
	}
	function get_class_list(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT s.* FROM students s inner join `class` c on c.id = s.class_id inner join class_unit cs on cs.class_id = c.id where cs.id = '$class_unit_id' ");
		if(isset($att_id)){
	...
	function get_att_record(){
		extract($_POST);
		$get = $this->db->query("SELECT s.* FROM students s inner join `class` c on c.id = s.class_id inner join class_unit cs on cs.class_id = c.id where cs.id = '$class_unit_id' ");
		$record = $this->db->query("SELECT ar.*,a.class_unit_id FROM attendance_record ar inner join attendance_list a on a.id =ar.attendance_id where a.class_unit_id='$class_unit_id' and a.doc = '$doc' ");
		$data = array();
		while($row = $get->fetch_assoc()){
			$data['data'][] = $row;
		}
		...
		$student_id = array_column($data['record'], 'student_id');
		array_multisort($student_id, SORT_ASC, $data['record']);



		$qry = $this->db->query("SELECT s.unit,co.course,concat(c.level,'-',c.section) as `class` FROM class_unit cs inner join class c on c.id = cs.class_id inner join units s on s.id = cs.unit_id inner join courses co on co.id = c.id where cs.id = {$class_unit_id} ");
	...
	function get_att_report(){
		extract($_POST);
		$get = $this->db->query("SELECT s.* FROM students s inner join `class` c on c.id = s.class_id inner join class_unit cs on cs.class_id = c.id where cs.id = '$class_unit_id' ");
		$record = $this->db->query("SELECT ar.*,a.class_unit_id FROM attendance_record ar inner join attendance_list a on a.id =ar.attendance_id where a.class_unit_id='$class_unit_id' and date_format(a.doc,'%Y-%m') = '$doc' ");
		$data = array();
		while($row = $get->fetch_assoc()){
			$data['data'][] = $row;
		}
		...
		$noc = $this->db->query("SELECT * FROM attendance_list where class_unit_id='$class_unit_id' and date_format(doc,'%Y-%m') = '$doc' ");
	...
	function save_attendance(){
		extract($_POST);
		$data  = " class_unit_id = '$class_unit_id' ";
		$data .= ", doc = '$doc' ";
		$data2  = " class_unit_id = '$class_unit_id' ";
		$data2 .= "and doc = '$doc' ";
