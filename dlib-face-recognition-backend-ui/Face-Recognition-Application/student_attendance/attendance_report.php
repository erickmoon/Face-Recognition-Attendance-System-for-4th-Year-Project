				...
				<form id="manage-attendance">
					...
					<div class="row justify-content-center">
						...
						<div class="col-sm-4">
				            <select name="class_unit_id" id="class_unit_id" class="custom-select select2 input-sm">
				                ...
				                ?>
				                <option value="<?php echo $row['id'] ?>" data-cid="<?php echo $row['id'] ?>" <?php echo isset($class_unit_id) && $class_unit_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['class'].' '.$row['unit']. ' [ '.$row['fname'].' ]' ?></option>
				                <?php endwhile; ?>
				            </select>
						</div>
					...
					<div class="row">
						...
						<div class="col-md-12"  style="display: none" id="submit-btn-field">
							<center>
								<button class="btn btn-success btn-sm col-sm-3" type="button" id="btnExport"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export to excel</button>
...
function export_file(){
	$("#btnExport").click(function(e) {
	    window.open('data:application/vnd.ms-excel,' + $('#table_clone').html());
	    e.preventDefault();
	});
}
setTimeout(function() { export_file(); }, 5000);
</script>
...
<script>
	...
	$('#filter').click(function(){
		...
		$.ajax({
			...
			method:'POST',
			data:{class_unit_id:$('#class_unit_id').val(),doc:$('#doc').val()},
			success:function(resp){
				if(resp){
					...
					_table.find('.unit').text(!!resp.details.unit ? resp.details.unit : '')
