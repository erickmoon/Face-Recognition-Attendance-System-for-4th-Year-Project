<?php
$conn = new mysqli('localhost:3305','root','','student_attendance_db')or die("Could not connect to mysql".mysqli_error($con));
...
$stmt = $conn->prepare("SELECT `class_id` FROM `class_unit` WHERE `id` = ?");
$stmt->bind_param("s",$class_unit_id);
$stmt->execute();
$stmt->store_result();
...
$stmt = $conn->prepare("SELECT `id` FROM `attendance_list` WHERE `class_unit_id` = ? AND `doc` = ? LIMIT 1");
$stmt->bind_param("ss",$class_unit_id,$date_selected);
$stmt->execute();
$stmt->store_result();
...
if ($attendance_list_exists == null) {
	$stmt = $conn->prepare("INSERT INTO `attendance_list` (`class_unit_id`,`doc`)VALUES(?,?)");
	$stmt->bind_param("ss",$class_unit_id,$date_selected);
    if($stmt->execute()){
        $stmt->insert_id;
        $array_result=array();
